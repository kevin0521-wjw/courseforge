...
- ⏳ **考试与事件倒计时**：在设置里登记考试和自定义事件（名称 + 日期 + 时间），首页紧跟「今日」面板出现一条倒计时 chip——今天的考试红色高亮，考试用警示色、普通事件用信息色。7 天内的考试还会自动出现在**系统托盘提示**和**桌面挂件**上；考完试自动消失，不用手动清理。事件跟学期走，切学期、备份、导出都一起带上
...
- 🧪 **自带测试**：539 项 `node --test` 测试（含 jsdom 全流程与真机页面夹具回归），CI 自动跑
...
> 当前 **539 项测试全部通过**；变异测试（`npm run check:mutation`，73 条变异）
> **护栏有效 70 · 假护栏 0 · 等价变异 1 · 跳过 2 · 无效判定 0** —— 除已登记的
> 等价变异与 2 条模式失配（源码改动后锚点没对上）外，每一处被删掉的代码都能让某条断言变红。
> 「假护栏 0」是硬要求：如果删掉某段代码测试仍全绿，就说明它没被守护，
> 要么补断言、要么显式登记为等价变异并写清理由，不能放着不管。
> 另：每条变异的测试执行带 180 秒超时 —— 若某条变异让测试死循环（真出过：

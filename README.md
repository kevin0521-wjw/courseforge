...
- 🧪 **自带测试**：512 项 `node --test` 测试（含 jsdom 全流程与真机页面夹具回归），CI 自动跑
...
> 以便人眼复核那块挂件到底长什么样（预览图就是功能自己产出的，不是另画的示意图）。
>
...
> 当前 **512 项测试全部通过**；变异测试（`npm run check:mutation`，68 条变异）
> **假护栏 0** —— 除已登记的等价变异与 2 条模式失配（源码改动后锚点没对上）外，
> 每一处被删掉的代码都能让某条断言变红。

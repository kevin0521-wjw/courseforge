...
| 来源 | 用法 | 说明 |
| --- | --- | --- |
| 🏫 教务直连 | 桌面端：填教务系统网址 → 打开窗口登录 → 进入课表页 → 「读取当前页课表」 | 免复制粘贴，直接解析课表页面；**仅桌面端**（网页版受同源策略限制） |
...
> 由 Electron 主进程开一个独立窗口让你自己登录（CourseForge 不接触账号密码），
> 需要时才把你当前那一页的 HTML 交回页面解析。课表放在 iframe 里也能读到（同源 iframe 会一并取回）。
...
│       ├── edu-html.js     # 教务系统 HTML 课表解析：网格/转置/列表三种版面（纯函数，可单测）
...
│   ├── main.js             # 主进程：窗口创建 + 教务系统直连（代取页面，绕开浏览器跨域）
